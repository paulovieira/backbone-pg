...
Backbone.Collection.prototype.execute = function(options) {
    ...
    if (options.changeCase === "undefined") {
        options.changeCase = _.result(collection, "changeCase");
    }
        ...
        .then(
            function(resp) {
                ...
                if (_.isFunction(options.parse)) {
                    resp = options.parse(resp, options);
                }
...
Backbone.syncPostgres = function(entity, options) {
    ...
    options.connection = _.result(options, "connection") || _.result(entity, 'connection') || propertyError('connection');

    // same for the query object
    options.query = _.result(options, 'query') || _.result(entity, 'query') || propertyError('query');
